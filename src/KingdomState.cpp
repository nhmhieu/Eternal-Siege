...
void KingdomState::onEnter(){
 ...
 textures.loadTexture(std::string(Player::TEXTURE_KEY),"assets/images/PlayerMage.png");
 textures.loadTexture("PlayerMageWalk","assets/images/animations/PlayerMage_Walk.png");
 textures.loadTexture("GuardWalk","assets/images/animations/Guard_Walk.png");textures.loadTexture("CitizenWalk","assets/images/animations/Citizen_Walk.png");
 textures.loadTexture("CastleGate","assets/images/kingdom/CastleGate.png");
 textures.loadTexture("KingdomWorldOpen","assets/images/kingdom/kingdom_world_open.png");
...
void KingdomState::update(float dt){
 dt=std::min(dt,1.f/30.f); elapsed+=dt;
 ...
 const sf::Vector2f lights[]={{84,875},{430,410},{845,520},{1300,205}};auto tintAt=[&](sf::Vector2f p){float glow=0;for(const auto&l:lights){const float distance=std::hypot(p.x-l.x,p.y-l.y);glow=std::max(glow,std::clamp(1.f-distance/145.f,0.f,1.f));}return sf::Color(static_cast<std::uint8_t>(205+35*glow),static_cast<std::uint8_t>(211+22*glow),static_cast<std::uint8_t>(220-20*glow));};if(avatar)avatar->setPresentationTint(tintAt(position));for(auto&npc:npcs)npc->setTint(tintAt(npc->getPosition()));
...
void KingdomState::render(sf::RenderWindow& w){
 ...
 drawText(w,"WASD Move    E Interact    F3 Navigation    ESC Menu",{28,684},14,{220,224,215,205});
