...
KingdomMap::KingdomMap(){
 ...
 waterZones={{{865,785},{335,132}},{{1090,690},{330,227}},{{1180,410},{255,350}},{{1390,410},{256,507}}};
...
bool KingdomMap::isWalkable(sf::Vector2f p)const{return canStandAt(p,8,true);}
bool KingdomMap::isNpcWalkable(sf::Vector2f p)const{return canStandAt(p,8,true);}
...
bool KingdomMap::inWalkableRegion(sf::Vector2f p)const{if(isOnBridge(p))return true;for(const auto&r:walkableRegions)if(r.bounds.contains(p))return true;return false;}
bool KingdomMap::blocked(sf::Vector2f p,float r,bool gateOpen)const{return blocked(sf::FloatRect{{p.x-r,p.y-r},{2*r,2*r}},gateOpen);}bool KingdomMap::blocked(sf::FloatRect footprint,bool gateOpen)const{const auto samplePoint=[&](std::size_t x,std::size_t y){return sf::Vector2f{footprint.position.x+footprint.size.x*(float(x)/2.f),footprint.position.y+footprint.size.y*(float(y)/2.f)};};for(std::size_t y=0;y<3;++y)for(std::size_t x=0;x<3;++x){const sf::Vector2f probe=samplePoint(x,y);if(!inWalkableRegion(probe)&&!isOnBridge(probe))return true;}const sf::Vector2f center=footprint.position+footprint.size*0.5f;for(const auto&o:obstacles)if(footprint.findIntersection(o).has_value())return true;const sf::Vector2f a{1030,655},b{1390,880},ab=b-a;const float length=std::hypot(ab.x,ab.y);const sf::Vector2f normal{-ab.y/length,ab.x/length};auto nearRail=[&](float side,const sf::Vector2f&p){const auto ra=a+normal*side,rb=b+normal*side,rab=rb-ra,ap=p-ra;const float t=std::clamp((ap.x*rab.x+ap.y*rab.y)/(rab.x*rab.x+rab.y*rab.y),0.f,1.f);const auto closest=ra+rab*t;return std::hypot((p-closest).x,(p-closest).y)<10.f;};for(std::size_t y=0;y<3;++y)for(std::size_t x=0;x<3;++x){const sf::Vector2f p=samplePoint(x,y);if(nearRail(-61.f,p)||nearRail(61.f,p))return true;}if(!isOnBridge(center,0)){for(const auto&w:waterZones)if(footprint.findIntersection(w).has_value())return true;}if(!gateOpen&&footprint.findIntersection(GATE_BLOCKER).has_value())return true;return false;}
...
sf::Vector2f KingdomMap::resolveMovementWithActors(sf::Vector2f p,sf::Vector2f d,const std::vector<sf::Vector2f>&actors,float r,float actorRadius,bool gateOpen)const{const int steps=std::max(1,int(std::ceil(std::hypot(d.x,d.y)/10)));const auto step=d/float(steps);auto clear=[&](sf::Vector2f candidate){if(blocked(candidate,r,gateOpen))return false;for(const auto&a:actors)if(std::hypot(candidate.x-a.x,candidate.y-a.y)<r+actorRadius)return false;return true;};for(int i=0;i<steps;++i){const sf::Vector2f x{p.x+step.x,p.y};if(clear(x))p.x=x.x;const sf::Vector2f y{p.x,p.y+step.y};if(clear(y))p.y=y.y;}return p;}
bool KingdomMap::isBlocked(sf::Vector2f p,float r,bool gateOpen)const{return blocked(p,r,gateOpen);}
bool KingdomMap::canStandAt(sf::Vector2f p,float r,bool gateOpen)const{return p.x>=0&&p.y>=0&&p.x<WORLD_SIZE.x&&p.y<WORLD_SIZE.y&&!blocked(p,r,gateOpen);}
bool KingdomMap::canStandAt(sf::FloatRect footprint,bool gateOpen)const{const sf::Vector2f end=footprint.position+footprint.size;return footprint.position.x>=0&&footprint.position.y>=0&&end.x<=WORLD_SIZE.x&&end.y<=WORLD_SIZE.y&&!blocked(footprint,gateOpen);}
bool KingdomMap::canInteract(sf::Vector2f p)const{return canStandAt(p,8,true)&&std::hypot(p.x-CAVE_CENTER.x,p.y-CAVE_CENTER.y)<=INTERACTION_RADIUS;}
...
bool KingdomMap::isNpcFootprintWalkable(sf::FloatRect b)const{const auto c=b.position+b.size/2.f;const float radius=std::max(b.size.x,b.size.y)*.5f;return canStandAt(c,radius,true);}
