...
GameplayState::GameplayState(StateMachine& machine, sf::RenderWindow& window, const std::vector<sf::Vector2i>& allyPositions)
    : stateMachine(machine), window(window), map(15, 15) {
    TextureManager::getInstance().loadTexture("Ash", "assets/images/Ash.png");
    ...
    for (const auto& name : allyTextureNames) {
        TextureManager::getInstance().loadTexture(name, "assets/images/" + name + ".png");
    }
    int allyindex = 0;
    ...
    for (const auto& pos : allyPositions) {
        ...
        auto ally = std::make_unique<Ally>(x, y, textureName);
...
void GameplayState::update(float dt) {
    ...
    if (player->isDead()) {
        ...
        stateMachine.changeState(std::make_unique<GameOverState>(stateMachine, window));
        return;
    }
    ...
    if (waveManager.isGameCompleted() && monsters.empty()) {
        ...
        stateMachine.changeState(std::make_unique<WinState>(stateMachine, window));
        return;
    }
}
