...
void KingdomState::update(float dt){
 ...
 const float len=std::hypot(input.x,input.y);const auto before=position;if(len>0){input/=len;facing=input;std::vector<sf::Vector2f>actorFeet;actorFeet.reserve(npcs.size());for(const auto&n:npcs)actorFeet.push_back(n->getFootPosition());position=map.resolveMovementWithActors(position,input*230.f*dt,actorFeet,14,12,!gate.blocksPassage());}const auto actual=position-before;if(avatar){avatar->setPosition(position);avatar->updateNonCombatPresentation(dt,actual);}
...
void KingdomState::render(sf::RenderWindow&w){
 ...
 if(renderer)renderer->drawOccluder(*sceneTarget,KingdomRenderer::BRIDGE_FAR_INDEX);
 struct Item{float key;int kind;std::size_t index;};std::vector<Item>items{{position.y,0,0}};for(std::size_t i=0;i<npcs.size();++i)items.push_back({npcs[i]->getSortKey(),1,i});if(renderer)for(std::size_t i=0;i<KingdomRenderer::OCCLUDER_COUNT;++i)if(i!=KingdomRenderer::BRIDGE_FAR_INDEX&&i!=KingdomRenderer::BRIDGE_NEAR_INDEX)items.push_back({renderer->occluderSortKey(i),2,i});std::stable_sort(items.begin(),items.end(),[](auto&a,auto&b){return a.key<b.key;});
 auto shadow=[&](sf::Vector2f p,float scale){if(map.isWater(p))return;if(const auto*t=textures.findTexture("KingdomContactShadow")){sf::Sprite s(*t);s.setOrigin({64,32});s.setPosition(p);s.setScale({scale,scale*.72f});s.setColor({255,255,255,std::uint8_t(245-75*dayNight.nightStrength())});sceneTarget->draw(s);}};
 for(auto&i:items){if(i.kind==0&&avatar){shadow(position,.62f);avatar->draw(*sceneTarget);}else if(i.kind==1){shadow(npcs[i.index]->getPosition(),.48f);npcs[i.index]->draw(*sceneTarget);}else if(renderer)renderer->drawOccluder(*sceneTarget,i.index);}if(renderer)renderer->drawOccluder(*sceneTarget,KingdomRenderer::BRIDGE_NEAR_INDEX);
 ...
 if(debugCollision){
  ...
  for(const auto&r:map.getWalkableRegions())rectangle(r.bounds,{45,210,95,28},{75,235,125,105});
  ...
  for(const auto&n:npcs)rectangle(n->getFootCollider(),{255,145,35,80},{255,175,65,235});
  sf::CircleShape interaction(KingdomMap::INTERACTION_RADIUS);interaction.setOrigin({KingdomMap::INTERACTION_RADIUS,KingdomMap::INTERACTION_RADIUS});interaction.setPosition(KingdomMap::CAVE_CENTER);interaction.setFillColor({180,70,255,18});interaction.setOutlineColor({210,110,255,170});interaction.setOutlineThickness(2);sceneTarget->draw(interaction);
 ...
 if(debugCollision)drawText(w,"F3 NAV  foot ("+std::to_string(int(position.x))+", "+std::to_string(int(position.y))+")  green=walkable  red=solid/rail  blue=water  cyan=deck  orange=NPC  purple=catacomb",{405,30},14,{255,245,190});
